...
5. **Align with web versions** for shared business libs (hook-form, zod, swr, axios, i18next 25, react-i18next 15, dayjs). See plan §2 for exact pins. Do NOT upgrade i18next to 26+ without a paired upgrade in the web project. `es-toolkit` is deferred to Stage 2 — its root entry (`AbortError` → `DOMException`) crashes under Hermes, and Stage 1 only needs a single `kebabCase` (lives in `src/utils/kebab-case.ts`). When it returns in Stage 2, prefer narrow submodule imports (`es-toolkit/array`, etc.).
